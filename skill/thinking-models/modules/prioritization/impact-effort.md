...
2. **Big Bets（高成本高影響）**：值得做但風險高，要拆成小階段、先驗證假設（對應 MVP）。
3. **Fill-ins（低成本低影響）**：不急不緊，有空順手做，不要佔主力時間。
4. **Money Pit（高成本低影響）**：最該警覺的象限。很多公司的技術債、無人用的功能、過度工程的系統都埋在這裡。**果斷砍掉**。

跟艾森豪矩陣的差別：艾森豪用「重要 vs 緊急」管**個人時間**；Impact vs Effort 用「價值 vs 成本」管**專案 / 功能 / 方案**。
...
不適合時機：
- 只有一個選項、沒有比較對象——用 decision 模組的單項評估
- 價值無法估算（探索型研究、品牌投資）——硬套會扭曲判斷
- 風險才是主要考量（不是成本）——該用 Pre-mortem 或期望值模型
...
- **Big Bet 不切 MVP**：高成本高影響的東西直接全押，沒先驗證假設。要搭配 MVP（action 模組）逐步釋放風險。
- **不敢砍 Money Pit**：已經投入一半的專案（沉沒成本偏誤）很難砍。矩陣要配合「沉沒成本不是成本」的思維。

## 與其他模型的關係

- **+ MVP**（action 模組）：Big Bet 不能一次下注，要切 MVP 先驗證關鍵假設。
- **+ 80/20**：在每個象限內再跑 80/20，找出那 20% 真正關鍵的。
- **+ North Star**：Impact 的大小應該用 North Star 來衡量，不然每個人對「影響」的定義飄來飄去。
- **+ 艾森豪矩陣**：Impact vs Effort 排專案層級，艾森豪矩陣排每日執行層級，兩者互補。
- **+ 沉沒成本**（decision / bias 模組）：砍 Money Pit 的最大阻力就是沉沒成本偏誤，兩者搭配用。
- **+ 槓桿**（action 模組）：高 Impact / 低 Effort 的本質就是「高槓桿活動」。
